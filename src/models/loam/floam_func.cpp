...
#include "../../../include/lidar_localization/models/loam/floam_opt.h"
...
bool EdgeAnalyticCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {
            ...
            //dk add
            // Eigen::Vector3d ib = lp - last_point_b;
            // Eigen::Vector3d ia = lp - last_point_a;
            // Eigen::Matrix3d skew_ia = skew(ia);
            // Eigen::Vector3d p1 = ia.cross(ib);
            // Eigen::Matrix3d skew_ib = skew(ib);

            Eigen::Matrix<double, 1, 3> res = nu.transpose() * skew_re / (nu.norm() * re.norm());

            // J_se3.block<3,6>(0,0) = skew_re * dp_by_so3/de.norm();

            //dk add
            J_se3.block<1,6>(0,0) = res * dp_by_so3;
